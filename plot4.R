plot4<-function()   {
    file_data<-read.csv("household_power_consumption.txt",header=TRUE,sep=";")
    suppressMessages(library(dplyr))
    selected_dates<-filter(file_data,Date =="1/2/2007"| Date=="2/2/2007")
    ...
    with( hpdata, 
          plot(DateTime,
               ...
               pch=".", 
               lines(DateTime,y=Global_active_power,type="s")
               )
        )
